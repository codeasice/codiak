...
def render_account_overview():
    ...
    for account_type in sorted(accounts_by_type.keys()):
        ...
        # Find orphaned accounts (no parent and no children)
        orphaned_accounts = []
        for account in root_accounts:
            # Check if this account has any children
            has_children = any(acc['parent_id'] == account['id'] for acc in accounts)
            if not has_children:
                orphaned_accounts.append(account)

        # Display orphaned accounts
        if orphaned_accounts:
            st.markdown("#### 📂 Orphaned Accounts (No Parent, No Children)")

            # Create table for orphaned accounts
            orphaned_table_data = []
            orphaned_total = 0

            for account in sorted(orphaned_accounts, key=lambda x: x['name']):
                ...
                orphaned_table_data.append(row_data)

            if orphaned_table_data:
                st.markdown(f"**Total: {format_amount(int(orphaned_total))}**")
                df = pd.DataFrame(orphaned_table_data)
